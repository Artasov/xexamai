...
- [Как использовать локально](#как-использовать-локально)
...
5. Если вы используете `Google AI`, то можете переключить `Transcription Type` на `Stream`, и тогда `Google` покажет вам распознанный текст, который вы можете отправить для получения ответа в нужный момент.
...
> Локальная потоковая передача как в Google недоступна, но это не страшно

...
- Standard OpenAI and Google requests are sent by the Rust host so long-lived provider API keys do not need to enter renderer state. OpenAI Realtime and Google Live are deliberate exceptions for the network path, but not for long-lived key handling: Rust uses the stored provider key to obtain a short-lived client secret or temporary token, and returns only that temporary credential to the renderer for the provider's direct Live WebSocket.
...
In buffered/local transcription modes, capture by itself does not send audio remotely. OpenAI Realtime and Google Live are the explicit exceptions: when either Live transcription model is selected, starting recording also starts continuous audio transfer to that provider until recording stops or the transcription mode/model changes. Other remote transfers start when the user sends a buffered fragment, asks a remote model, captures a screenshot for analysis, signs in, checks account features, sends feedback, or downloads an update.

| Mode or action | Leaves the device? | Data and destination | Local/server persistence |
| --- | --- | --- | --- |
| Audio capture and rolling buffer | No in buffered/local modes; yes when OpenAI Realtime or Google Live is selected | Microphone, system audio, or their mix stays in application memory until another action consumes it. In either Live mode, starting capture immediately begins the continuous transfer described below. | XEXAMAI does not intentionally write the raw rolling buffer to disk. It is limited to the actual configured duration and is cleared when the recording session is torn down. This is not a forensic guarantee against OS paging or crash capture. |
...
| OpenAI buffered transcription/LLM | Yes | The chosen audio fragment for transcription and/or prompts, system instruction, recent context, and model parameters for generation go to `https://api.openai.com`. A screenshot is included only for an explicit screenshot-analysis action. | Provider-side processing, retention, abuse monitoring, and account billing follow the user's OpenAI account and OpenAI terms. |
| OpenAI Realtime (`gpt-live-transcribe`) | Yes, continuously while the Realtime session is active | Rust uses the stored long-lived OpenAI key only to request a short-lived client secret. The renderer receives that secret and streams selected audio and the transcription prompt directly to `wss://api.openai.com`; language hints are omitted so the service can automatically handle multilingual speech and code-switching. | XEXAMAI writes recognized text into the question field and can later store it in local history when sent. OpenAI controls service-side processing, retention, abuse monitoring, and billing under the user's OpenAI account and OpenAI terms. |
...
- OpenAI and Google API keys are stored through the OS credential store (Windows Credential Manager, macOS Keychain, or the available Linux Secret Service implementation). Live renderer sessions receive only short-lived provider credentials, not those stored API keys. XEXAMAI does not claim stronger protection than the active OS account and credential-store backend provide.

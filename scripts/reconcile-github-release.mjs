...
async function githubJson(
  url,
  { method = "GET", payload, expected = [200] } = {},
) {
...
async function expectedAssets() {
  ...
  for (const platform of fs
    ...
    for (const entry of fs.readdirSync(filesDirectory, {
      withFileTypes: true,
    })) {
...
async function getReleaseByTag() {
  ...
  if (!response.ok) {
    throw new Error(
      `Could not read GitHub release ${tag}: HTTP ${response.status} ${await response.text()}`,
    );
  }
...
async function findReleaseByTag() {
  ...
  for (let page = 1; page <= 20; page += 1) {
    ...
    if (page === 20) {
      throw new Error(
        `Could not exhaustively search GitHub releases for draft ${tag}`,
      );
    }
  }
  if (matches.length > 1) {
    throw new Error(
      `Multiple GitHub releases unexpectedly reference tag ${tag}`,
    );
  }
...
async function createDraftRelease() {
  ...
  throw new Error(
    `Could not create draft GitHub release ${tag}: HTTP ${response.status} ${await response.text()}`,
  );
}
...
async function listAssets(releaseId) {
  ...
  for (let page = 1; ; page += 1) {
    ...
    if (!Array.isArray(pageAssets))
      throw new Error("GitHub release assets response is not an array");
...
async function actualAssetHash(asset) {
  ...
  if (!response.ok) {
    throw new Error(
      `Could not download existing GitHub asset ${asset.name}: HTTP ${response.status}`,
    );
  }
...
function assertExactAssets(inspection) {
  if (inspection.repair.length > 0) {
    const details = inspection.repair
      .map(
        ({ asset, reason }) =>
          `${String(asset.name ?? asset.id ?? "unknown")} (${reason})`,
      )
      .join(", ");
    throw new Error(
      `GitHub release ${tag} has non-matching assets: ${details}`,
    );
  }
  if (inspection.missing.length > 0) {
    throw new Error(
      `GitHub release ${tag} is missing assets: ${inspection.missing.join(", ")}`,
    );
  }
}
...
function assertPublishedMetadata(
  ...
  if (release.draft)
    throw new Error(`GitHub release ${tag} is unexpectedly still a draft`);
  if (release.tag_name !== tag)
    throw new Error(`GitHub release tag mismatch for ${tag}`);
  if (Boolean(release.prerelease) !== prerelease) {
    throw new Error(
      `Published GitHub release ${tag} has the wrong prerelease state`,
    );
  }
  if (
    requireExpectedContent &&
    (release.name !== tag || release.body !== body)
  ) {
    throw new Error(
      `Published GitHub release ${tag} metadata differs from the expected release`,
    );
  }
}
...
async function deleteAsset(asset, reason) {
  if (!Number.isInteger(asset.id)) {
    throw new Error(
      `Cannot repair GitHub release asset ${String(asset.name)} without a numeric id`,
    );
  }
  ...
  console.log(
    `Removed ${reason} draft GitHub release asset ${String(asset.name)}.`,
  );
}
...
async function uploadAsset(release, name, asset) {
  ...
  if (response.status !== 201) {
    throw new Error(
      `Could not upload GitHub asset ${name}: HTTP ${response.status} ${await response.text()}`,
    );
  }
...
if (!release.draft) {
  ...
  console.log(
    `GitHub release ${tag} is published with the exact expected asset set.`,
  );
} else {
  const inspection = await inspectAssets(release.id, expected);
  for (const item of inspection.repair)
    await deleteAsset(item.asset, item.reason);

  const stillDraft = await getReleaseById(release.id);
  if (!stillDraft || stillDraft.id !== release.id || !stillDraft.draft) {
    throw new Error(
      `GitHub release ${tag} changed while its draft assets were being reconciled`,
    );
  }
  release = stillDraft;
  const missing = [...expected.entries()].filter(
    ([name]) => !inspection.matching.has(name),
  );
  for (const [name, asset] of missing) await uploadAsset(release, name, asset);

  assertExactAssets(await inspectAssets(release.id, expected));
  release = await githubJson(
    `${apiBase}/repos/${repositoryPath}/releases/${release.id}`,
    {
      method: "PATCH",
      payload: {
        name: tag,

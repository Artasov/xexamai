...
async function fakeS3(initial: Record<string, string>, missingStatus = 404) {
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.method === "GET") {
      ...
      response.setHeader(
        "ETag",
        etags.get(pathname) ?? assignEtag(pathname, body),
      );
      response.end(body);
      return;
    }
  ...
  if (!address || typeof address === "string")
    throw new Error("Fake S3 did not bind a TCP port");
  return {
...
async function fakeGitHub(
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (
      request.method === "GET" &&
      pathname === "/repos/acme/xexamai/releases/tags/v2.5.0"
    ) {
      if (options.readStatus && options.readStatus !== 200) {
        writeJson(response, options.readStatus, {
          message: "injected read failure",
        });
        return;
      }
      if (!release || (options.tagLookupHidesDrafts && release.draft)) {
    ...
    if (
      request.method === "GET" &&
      pathname === "/repos/acme/xexamai/releases"
    ) {
      writeJson(response, 200, release ? [releaseJson()] : []);
      return;
    }
    if (
      request.method === "GET" &&
      pathname === "/repos/acme/xexamai/releases/1"
    ) {
      if (!release) {
        writeJson(response, 404, { message: "Not Found" });
        return;
    ...
    if (
      ...
      const payload = JSON.parse(
        (await readBody(request)).toString(),
    ...
    if (
      request.method === "GET" &&
      pathname === "/repos/acme/xexamai/releases/1/assets"
    ) {
      writeJson(response, 200, [...assets.values()].map(assetJson));
      return;
    }
    const assetMatch = pathname.match(
      /^\/repos\/acme\/xexamai\/releases\/assets\/(\d+)$/,
    );
  ...
  const address = server.address();
  if (!address || typeof address === "string")
    throw new Error("Fake GitHub did not bind a TCP port");
  endpoint = `http://127.0.0.1:${address.port}`;
  return {
    endpoint,
    assets,
...
function createUploadFixture(directory: string, content = "signed artifact") {
  ...
  fs.writeFileSync(
    path.join(publication, "publication.json"),
    JSON.stringify({
      channel: "stable",
  ...
  fs.writeFileSync(
    path.join(publication, "latest.json"),
    JSON.stringify({
      version: "2.5.0",
      notes: "current notes",
...
describe("release artifact scripts", () => {
  ...
  it("classifies every valid SemVer prerelease and preserves SemVer ordering", () => {
    ...
    expect(compareSemver("2.5.0-preview.2", "2.5.0-preview.10")).toBeLessThan(
      0,
    );
    expect(
      compareSemver(
        "2.5.0-preview.9007199254740993",
        "2.5.0-preview.9007199254740994",
      ),
    ).toBeLessThan(0);
    expect(compareSemver("2.5.0-rc.1", "2.5.0")).toBeLessThan(0);
    expect(() => isPrerelease("2.5")).toThrow(/Invalid semantic version/);
    expect(() => isPrerelease("2.5.0-rc.01")).toThrow(
  ...
  it("selects a signed native Windows updater payload", () => {
    ...
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_x64-setup.exe"),
      "installer",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_x64-setup.exe.sig"),
      "signature",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_x64_en-US.msi"),
      "msi installer",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_x64_en-US.msi.sig"),
      "msi signature",
    );

    runScript(
      "prepare-release-artifacts.mjs",
      [path.dirname(bundle), output, "windows"],
      {
        RUNNER_ARCH: "X64",
      },
    );
    ...
    const metadata = JSON.parse(
      fs.readFileSync(path.join(output, "windows", "metadata.json"), "utf8"),
    );
  ...
  it("selects the signed native AppImage as the Linux updater payload", () => {
    ...
    fs.mkdirSync(bundle, { recursive: true });
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_amd64.AppImage"),
      "updater bundle",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_amd64.AppImage.sig"),
      "signature",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_amd64.deb"),
      "deb package",
    );
    fs.writeFileSync(
      path.join(bundle, "XEXAMAI_2.4.1_amd64.deb.sig"),
      "deb signature",
    );

    runScript(
      "prepare-release-artifacts.mjs",
      [path.dirname(bundle), output, "linux"],
      {
        RUNNER_ARCH: "X64",
      },
    );
    ...
    const metadata = JSON.parse(
      fs.readFileSync(path.join(output, "linux", "metadata.json"), "utf8"),
    );
    expect(metadata).toMatchObject({
      platform: "linux",
      updaters: [
  ...
  it("builds a beta channel manifest only when every desktop platform is signed", () => {
    ...
    for (const [folder, updaters] of [
      ...
      [
        "macos",
        [{ target: "darwin-aarch64-app", updaterFile: "app.app.tar.gz" }],
      ],
      [
        "linux",
        [
      ...
      fs.writeFileSync(
        path.join(platformDirectory, "metadata.json"),
        JSON.stringify({
          platform: folder,
          updaters: updaters.map((updater) => ({
            ...updater,
            signature: `${updater.target}-signature`,
          })),
    ...
    runScript(
      ...
      [artifacts, "v2.5.0-beta.1", notes, output, "2026-08-20T18:23:45+04:00"],
      {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(output, "latest.json"), "utf8"),
    );
    const publication = JSON.parse(
      fs.readFileSync(path.join(output, "publication.json"), "utf8"),
    );
    ...
    expect(fs.readFileSync(path.join(secondOutput, "latest.json"))).toEqual(
      fs.readFileSync(path.join(output, "latest.json")),
    );
  });
  ...
  it("routes non-alpha prereleases to the beta manifest", () => {
    ...
    for (const [folder, updaters] of [
      ...
      [
        "macos",
        [{ target: "darwin-aarch64-app", updaterFile: "macos.tar.gz" }],
      ],
      [
        "linux",
        [
    ...
    runScript(
      "build-update-manifest.mjs",
      [artifacts, "v2.5.0-preview.1", notes, output, "2026-08-20T14:23:45Z"],
      {
        S3_ENDPOINT: "https://updates.example.test",
        S3_BUCKET: "desktop-releases",
      },
  ...
  it("keeps partially published tag objects immutable and resumes identical uploads", async () => {
    ...
    expect(
      server.puts.find((item) => item.path.endsWith("/windows/app.bin")),
    ).toBeUndefined();
    expect(
      server.puts.find((item) => item.path.endsWith("/v2.5.0/latest.json"))
        ?.ifNoneMatch,
    ).toBe("*");
    expect(
      server.puts.find((item) => item.path.endsWith("/xexamai/latest.json"))
        ?.ifNoneMatch,
    ).toBe("*");
  });
  ...
  it("publishes safely when missing S3 objects are hidden behind HTTP 403", async () => {
    ...
    expect(
      JSON.parse(
        server.objects.get("/bucket/xexamai/latest.json")?.toString() ?? "{}",
      ).version,
    ).toBe("2.5.0");
  });
  ...
  it("refuses to overwrite a mismatched object from a partial release", async () => {
    ...
    expect(server.objects.get(immutablePath)?.toString()).toBe(
      "different build",
    );
    expect(
      server.puts.find((item) => item.path === immutablePath),
    ).toBeUndefined();
  });
  ...
  it("adopts the first immutable tag manifest byte-for-byte on a historical rerun", async () => {
    ...
    expect(
      fs.readFileSync(path.join(fixture.publication, "latest.json"), "utf8"),
    ).toBe(canonicalManifest);
    expect(
      fs.readFileSync(
        path.join(fixture.publication, "release-body.md"),
        "utf8",
      ),
    ).toBe(
    ...
    expect(
      server.puts.find((item) => item.path === tagManifestPath),
    ).toBeUndefined();
  });
  ...
  it("validates an existing tag manifest before writing any immutable release objects", async () => {
    ...
    expect(server.objects.has("/bucket/xexamai/v2.5.0/windows/app.bin")).toBe(
      false,
    );
  });
  ...
  it("retries a raced first channel publication with an ETag compare-and-swap", async () => {
    ...
    expect(
      JSON.parse(server.objects.get(channelPath)?.toString() ?? "{}").version,
    ).toBe("2.5.0");
  });
  ...
  it("never rolls a channel pointer back when a newer tag wins the ETag race", async () => {
    ...
    expect(
      server.puts.filter((item) => item.path === channelPath),
    ).toHaveLength(1);
    expect(
      JSON.parse(server.objects.get(channelPath)?.toString() ?? "{}").version,
    ).toBe("2.6.0");
  ...
  it("repairs a partial draft release, preserves matching bytes and publishes an exact set", async () => {
    ...
    const preservedId = [...server.assets.values()].find(
      (asset) => asset.name === "app.bin",
    )?.id;
    try {
      await runScriptAsync(
    ...
    expect(
      [...server.assets.values()].find((asset) => asset.name === "app.bin")?.id,
    ).toBe(preservedId);
    expect(
      [...server.assets.values()].map((asset) => asset.name).sort(),
    ).toEqual(["app.bin", "app.sig", "manual.txt"]);
  ...
  it("does not mutate a published release with a mismatched asset", async () => {
    ...
    expect([...server.assets.values()][0]?.content.toString()).toBe(
      "wrong application",
    );
